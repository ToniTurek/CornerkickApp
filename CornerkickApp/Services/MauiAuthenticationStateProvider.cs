...
namespace CornerkickApp.Services
{
    ...
    public class MauiAuthenticationStateProvider : AuthenticationStateProvider
    {
        ...
        private const int TokenExpirationBuffer = 60; //minutes
        ...
        private async Task<bool> UpdateAndValidateAccessTokenAsync()
        {
            try
            {
                ...
                if (_accessToken is null || now > _accessToken.AccessTokenExpiration)
                {
                ...
                var accessTokenExTime = _accessToken.AccessTokenExpiration.ToUniversalTime();
                if (now >= _accessToken.AccessTokenExpiration)
                {
